...
from models.model_PFPCN import _netlocalD,_netG_PCN
from datasets.dfaustDataset import DFaustDataset
from datasets.dfaustPreDataset import DFaustPreDataset
...
def train(point_netG, point_netD, trainLoader, testLoader, trainDataset, testDataset, opt):
    ...
    for epoch in range(0, opt.niter):
        ...
        schedulerD.step()
        schedulerG.step()
        print_log(f, "Learning Rate of G:" + str(schedulerG.get_last_lr()[0]))
        print_log(f, "Learning Rate of D:" + str(schedulerD.get_last_lr()[0]))
